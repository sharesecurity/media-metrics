...
import { Layers, RefreshCw, ExternalLink, ChevronDown, ChevronRight, ArrowRight, Tag } from 'lucide-react'
import { getClusters, runClustering } from '../utils/api'
import axios from 'axios'

const relabelClusters = (minSources = 2) =>
  axios.post('/api/clusters/relabel', null, { params: { min_sources: minSources } }).then(r => r.data)
...
export default function StoryClusters() {
  ...
  const handleRelabel = async () => {
    ...
    setRelabelMsg('')
    try {
      const res = await relabelClusters(minSources > 1 ? minSources : 2)
      setRelabelMsg(`Relabeling started for clusters with 2+ outlets. Labels update in background — refresh in ~2 min.`)
      // Auto-refresh clusters after a delay to pick up new labels
      setTimeout(() => { refetch(); setRelabeling(false) }, 90000)
    } catch (e) {
      setRelabelMsg('Error starting relabel job.')
      setRelabeling(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            disabled={relabeling}
            title="Use AI to regenerate cleaner topic labels for all clusters"
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-indigo-700 hover:bg-indigo-600 text-white text-sm transition-colors disabled:opacity-50"
          >
            <Tag size={13} className={relabeling ? 'animate-pulse' : ''} />
      ...
      {relabelMsg && (
        <div className="text-xs text-indigo-400 bg-indigo-900/20 border border-indigo-800/40 rounded-lg px-3 py-2">
          {relabelMsg}
        </div>
      )}
